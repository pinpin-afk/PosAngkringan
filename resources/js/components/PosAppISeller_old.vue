      ...
      <div class="px-6 py-4">
        <div class="flex items-center justify-between">
          ...
          <div class="flex items-center space-x-4">
            <!-- Draft Orders Button -->
            <button 
              @click="showDraftOrdersModal = true; loadDraftOrders()"
              class="flex items-center space-x-2 px-3 py-2 rounded-lg transition-colors duration-300 text-sm font-medium" 
              :class="isDarkMode ? 'bg-blue-600 hover:bg-blue-700 text-white' : 'bg-blue-100 hover:bg-blue-200 text-blue-700'"
            >
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v10a2 2 0 002 2h8a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"></path>
              </svg>
              <span class="hidden sm:inline">Draft Orders</span>
            </button>
  ...
  methods: {
    ...
    async addMemberFlow() {
      try {
        ...
        await axios.post('/api/kasir/members', {
          name,
          phone,
          email,
          address,
    ...
    async editMember(member) {
      try {
        ...
        await axios.put(`/api/kasir/members/${member.id}`, {
          name,
          phone,
          email,
          address,
    ...
    async deleteMember(member) {
      try {
        ...
        await axios.delete(`/api/kasir/members/${member.id}`);
