...
return new class extends Migration
{
    public function up(): void
    {
        ...
        if ($driver === 'pgsql') {
            // Drop existing check constraint if present, then add new one including 'transfer'
            // Try common constraint name
            DB::statement("ALTER TABLE orders DROP CONSTRAINT IF EXISTS orders_payment_method_check");
            // Also attempt to drop any constraint that mentions payment_method
    ...
    public function down(): void
    {
        ...
        if ($driver === 'pgsql') {
            // Revert check constraint to original (without 'transfer')
            DB::statement("ALTER TABLE orders DROP CONSTRAINT IF EXISTS orders_payment_method_check");
            $constraints = DB::select("
                SELECT conname FROM pg_constraint c
                JOIN pg_class t ON c.conrelid = t.oid
            ...
            DB::statement("ALTER TABLE orders ADD CONSTRAINT orders_payment_method_check CHECK (payment_method IN ('cash','card','qris'))");
            return;
        }
